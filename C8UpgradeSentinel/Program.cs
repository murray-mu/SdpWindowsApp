...
class FileWatcher {
    ...
    public static void StartC8EdgeUI() {
        ...
        if (dirs.Count > 1) {
            ...
            Log($"cannot start the UI. No C8Edge.exe found");
        } else {
            var zitiFiles = Directory.GetFiles(dirs[0], "C8Edge.exe", SearchOption.AllDirectories);

            foreach (var file in zitiFiles) {
                Console.WriteLine($"Found C8Edge at: {file}");
